...
const useChatStore = create<StoreContextType>()(
        ...
        try {
          ...
          switch (type) {
            ...
            case "getAllMessagesResponse": {
              ...
              //   convId =
              //     (first.conversationId as string | undefined) ??
              //     (first.convId as string | undefined);
              // }
              if (convId)
                set((s) => ({
                  state: reducer(s.state, {
            ...
            case "getProfileResponse": {
              ...
              if (prof && curUser && prof.id === curUser.id) {
                ...
                try {
                  if (
                    Object.prototype.hasOwnProperty.call(safeProf, "password")
                  ) {
                    try {
                      delete (safeProf as unknown as Record<string, unknown>)
                        .password;
                    } catch (e) {}
                  }
                } catch (e) {}
  ...
  setSearchResults: (users: User[]) => {
    set((s) => ({
      state: reducer(s.state, { type: "SET_SEARCH_RESULTS", users } as Action),
    }));
  },
  clearSearchResults: () => {
    set((s) => ({
